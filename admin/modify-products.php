...
$sqlfindproducts = "SELECT * FROM `products` ORDER BY `id`";
...
<html lang="en">
  <head>
  	<title>Kerry's K9's - Modify Products</title>
    <meta charset="utf-8">
...
<body>
    <?php include "inc/header.php"; ?>
        ...
        <center>
    <h2 class="mb-42">Modify Products</h2>
  ...
  <?php
    while ($rowfindproducts = $findproducts->fetch_assoc())
    {
        ...
        <td>$price</td>
        <td>$discountmsg</td>
        <td><input type='button' data-toggle='modal' id='cancel_btn' data-target='#product{$productid}' class='btn btn-warning' value='Manage Product' /></td>
        </tr>
        ";

        echo "<div class='modal fade' id='product{$productid}' tabindex='-1' role='dialog' aria-labelledby='product{$productid}' aria-hidden='true'>
        <div class='modal-dialog' role='document'>
          <div class='modal-content' style='width:150%;left:-10%;'>
            ...
            <p>Deleting the product cannot be undone.</p>
            </div>
            <div class='modal-footer'>
            ";
                   
           echo "
            <form action='change-product.php' method='post' role='form'>
            <input type='hidden' value='$productid' name='id' />
              <button type='submit' class='btn btn-success'>Modify Product</button>
            </form>

            <form action='php/delete-product.php' method='post' role='form'>
            <input type='hidden' value='$productid' name='id' />
